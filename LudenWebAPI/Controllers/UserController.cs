        ...
        public async Task<IActionResult> UpdateUser([FromForm] UpdateUserDTO dto)
        {
            try
            {
                ...
                if (dto.Avatar != null && dto.Avatar.Length > 0)
                {
                    ...
                    {
                        // Передаем null в качестве первого аргумента (currentFileId), так как UploadImageAsync разберется с удалением старого аватара сама
                        // userId должен быть ulong, не nullable ulong
                        var photoFile = await _fileService.UploadImageAsync(null, (ulong)userId, stream, dto.Avatar.FileName, dto.Avatar.ContentType, dto.Avatar.Length, true);
        ...
        public async Task<ActionResult<ICollection<ProductDto>>> GetUserProducts()
        {
            try
            {
                ...
                foreach (var p in products)
                {
                    ...
                    if (p.CoverFileId.HasValue)
                    {
                        var coverFile = await _fileService.GetImageFileByIdAsync(p.CoverFileId.Value);
                        if (coverFile != null)
                        {
                    ...
                    var files = p.Files?.Select(f => new ProductFileDto
                    {
                        Id = f.Id,
                        Path = f.Path,
                        FileName = f.FileName,
